...
export function LiveNotifications() {
  ...
  return (
    <div className="relative">
      ...
      {open && (
        <div className="absolute right-0 mt-2 w-[320px] max-h-[420px] overflow-y-auto rounded-lg border border-slate-200 bg-white shadow-xl z-50">
          <div className="px-3 py-2 border-b flex items-center justify-between">
            <div className="text-sm font-semibold">Live notifications</div>
            <span className={`text-[10px] uppercase ${connected ? 'text-green-600' : 'text-slate-400'}`}>
              {connected ? 'Live' : 'Offline'}
            </span>
          </div>
          <div className="divide-y">
            {visible.length === 0 && <div className="p-3 text-sm text-slate-500">No new activity.</div>}
            {visible.map((n) => (
